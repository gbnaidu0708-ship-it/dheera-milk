...
import { enumerateDays, monthLastDay, todayIso } from '@/lib/month'
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Mid-month proration: only schedule from start_date through the end of
    // the start_date's calendar month. Subsequent months are generated by a
    // monthly cron / admin job — never bill for days before the start date.
    const [y, m] = body.start_date.split('-').map(Number)
    const lastOfMonth = monthLastDay(y, m)
    const dates = enumerateDays(body.start_date, lastOfMonth)
    ...
    // Generate / refresh the first month's invoice so the customer sees the
    // prorated total immediately. Subsequent months invoice on schedule.
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    // Recalculate the current-month invoice so the change reflects everywhere.
    const now = new Date()
    await recalcInvoice(sb, profile.id, now.getFullYear(), now.getMonth() + 1)
